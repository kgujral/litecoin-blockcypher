...
public final class TransactionService extends AbstractService {
  ...
  // ie
  // https://api.blockcypher.com/v1/btc/test3/txs/09a228c6cf72989d81cbcd3a906dcb1d4b4a4c1d796537c34925feea1da2af35
   ...
   * 
   * @param transactionSkeleton
   *          input/output+value simple skeleton
   * @return Partially built transaction with the data to sign
   * @throws BlockCypherException
   *           In case there is some error, ie:
   *           <code>Address xxx with balance 0 does not have enough funds to transfer 510000.</code>
   */
  ...
  private IntermediaryTransaction postSkeletonTransaction(String lightTransaction, String id)
      throws BlockCypherException {
    return RestUtils.post(RestUtils.formatUrl(resourceUrl, endpointConfig, id), lightTransaction, id,
        IntermediaryTransaction.class);
  }

  private Transaction postTransaction(String signedTransaction, String id) throws BlockCypherException {
    return (RestUtils.post(RestUtils.formatUrl(resourceUrl, endpointConfig, id), signedTransaction, id,
        IntermediaryTransaction.class)).getTx();
  }
  ...
  /**
   * Create an Intermediary Transaction from skeleton. You will then need to sign
   * this transaction with your private key
   *
  ...
  public IntermediaryTransaction newTransaction(List<String> inputAddresses, List<String> outputAddresses,
    ...
    String transactionJson = buildTransactionJson(inputAddresses, outputAddresses, satoshis);
    return postSkeletonTransaction(transactionJson, "new");
  }

  public String buildTransactionJson(List<String> inputAddresses, List<String> outputAddresses, long satoshis) {
    ...
    return GsonFactory.getGson().toJson(transaction);
  }

  /**
   * Create a Funding multisig-2-of-3 Transaction from skeleton. You will then
   * need to sign the returned transaction
   * 
   * @param inputAddresses
   *          input addresses
   * @param outputAddresses
  ...
  /**
   * Send a signed transaction. This is the final step once you have signed an
